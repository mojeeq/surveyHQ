 ...
 *
 * The probe above says the launcher is the launcher. This says the launcher
 * can do its job here, which is a different question and the one that decides
 * whether R may run: Landlock needs Linux 5.13 or newer, and needs the three
 * landlock syscalls to reach the kernel. Docker's default seccomp profile
 * permits them, but a hardened host, an older kernel or a stricter runtime may
 * not - and the platform would rather say which of those it is than hand
 * somebody "R is not available" and let them guess.
 *
 * Only the version query is made, which allocates nothing and changes nothing
 * about this process, so the self-test is safe to run on every probe.
 */
static int selftest(void) {
    int abi = landlock_create(NULL, 0, LANDLOCK_CREATE_RULESET_VERSION);
    if (abi < 1) {
        fprintf(stderr,
                "landlock-unavailable: errno=%d (%s). Linux 5.13 or newer is "
                "required, and the landlock syscalls must reach the kernel.\n",
                errno, strerror(errno));
        return 1;
    }
    if (access("/usr/bin/Rscript", X_OK) != 0) {
        fprintf(stderr, "rscript-missing: /usr/bin/Rscript is not executable: %s\n",
                strerror(errno));
        return 1;
    }
...
int main(int argc, char **argv) {
    ...
    /* Asked before R is offered at all, so a host that cannot enforce the
       sandbox says so in the interface rather than at the first script. */
    if (argc == 2 && strcmp(argv[1], "--surveyhq-sandbox-selftest") == 0) {
        return selftest();
    }
